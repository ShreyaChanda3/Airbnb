...
#Research Question - factors to determine host is a super host
#Factors - host_response_time, host_response_rate, bedbath,bedrooms,beds, number_of_reviews, reviews_per_month and review_scores_rating
...
#don't run this
airbnb_df$host_is_superhost <- as.numeric(airbnb_df$host_is_superhost)
sum(is.na(airbnb$host_is_superhost))
##############
...
airbnb$host_response_time<- stri_replace_all_coll(airbnb$host_response_time,"within an hour","1")  #within an hour - 1
airbnb$host_response_time<- stri_replace_all_coll(airbnb$host_response_time,"within a few hours","2")  #within a few hours - 2
airbnb$host_response_time<- stri_replace_all_coll(airbnb$host_response_time,"within a day","3")  #within a day - 3
airbnb$host_response_time<- stri_replace_all_coll(airbnb$host_response_time,"a few days or more","4")  #within few days or more - 4
...
#Research question
#does being a super host and other factors impact the review scores rating

#
#being a superhost and other factors has an impact on the review scores value
...
#########################################################
#check for normality
qqnorm(airbnb$review_scores_value)
qqline(airbnb$review_scores_value)

qqnorm(airbnb$number_of_reviews)
qqline(airbnb$number_of_reviews)
#########################################################

#mann whitney test for normality
wilcox.test(airbnb$host_is_superhost,airbnb$review_scores_value, data=airbnb) 

#they are non identical populations
#########################


#regression model
model_LM <- lm(review_scores_value~host_is_superhost,data = airbnb)


# fit the quadratic Model
quadratic_model <- lm(review_scores_value~host_is_superhost+ host_is_superhost^2, data = airbnb)
...
library(car)
residualPlots(model_LM)



plot(airbnb$review_scores_value~airbnb$host_is_superhost)

# plot the observations
plot(airbnb$review_scores_value,airbnb$host_response_rate,
     col = "steelblue",
     pch = 20,
     xlab = "bedrooms", 
     ylab = "Review Scores",
     cex.main = 0.9,
     main = "Review Score vs. Host response rate")
abline(model_LM, 
       col = "red", 
       lwd = 2)
...
#you can see that it is significant at 1% level


plot(airbnb$host_is_superhost,airbnb$review_scores_value)
abline(model_LM)
...
#############
#plot the data 
plot(x = airbnb$review_scores_value, 
     y = airbnb$host_is_superhost,
     main = "Scatterplot for Superhost and review scores",
     xlab = "Review Score",
     ylab = "Superhost",
     pch = 20,
     ylim = c(-0.4, 1.4),
...
text(2.5, 0.9, cex = 0.8, "Superhost")
text(2.5, -0.1, cex= 0.8, "Not a superhost")


##################
# add estimated regression line of Logit models
x <- seq(1,10,length.out=10906)
length(x) == length(y_logit)

x <- seq(1,5,length.out=10906)
y_logit <- predict(model_logistic_1, list(review_scores_value = x), type = "response")
length(y_logit)
...
length(airbnb$review_scores_value)
length(airbnb$host_is_superhost)


ggplot(airbnb, aes(x=review_scores_value, y=host_is_superhost)) + geom_point() +
  stat_smooth(method="glm", color="green", se=FALSE,
              method.args = list(family=binomial)) + annotate("text", x=1, y=0.97, label= "Super Host")+annotate("text", x=2, y=0.10, label= "Not a super host")
...
#plot for our continous variables
par(mfrow = c(2,2)) 
...
exp(coef(model_logistic_8))
#1.221187e+00*10 - review scores_value
#1.085511e+00 *10 _host_response data
#1.005754e+00 *10  number of reviews
#9.571259e-01 *10 beds
#3.371197e+01  *10 review_scores_rating 
#1.012532e+00  *10  bedrooms  
#-0.0037366*10 reviews_per_month*
#-0.0268946* host_response_time
...
##########################
#review accuracy
typeof(airbnb_df$review_scores_rating)
sum(is.na(airbnb_df$review_scores_rating))
airbnb_df$review_scores_rating <- as.numeric(airbnb_df$review_scores_rating)

#drop na
airbnb_df <- drop_na(airbnb_df)

###########################


cor(airbnb_df$number_of_reviews,airbnb_df$host_is_superhost)

cor(airbnb_df$review_scores_rating,airbnb_df$host_is_superhost)

airbnb_df$superhost_true <- airbnb_df$host_is_superhost == 1
airbnb_df$superhost_false <- airbnb_df$host_is_superhost == 0


typeof(airbnb_df$superhost_true)
airbnb_df$superhost_true <- as.integer(airbnb_df$superhost_true)

airbnb_df$superhost_false <- as.integer(airbnb_df$superhost_false)


#avg when superhost is true
sum(airbnb_df$review_scores_rating[airbnb_df$superhost_true])
#13711.56


#avg when superhost is false
sum(airbnb_df$review_scores_rating[airbnb_df$superhost_false])
#69027.24
...
#Research question
# To cheeck if a superhost has higher review rating


#null hypotheses : A superhost does not have highe rating
#alternate hypotheses : A superhost has higher rating


#two sample t test
shapiro.test(airbnb_df$review_scores_rating)
shapiro.test(airbnb_df$review_scores_rating[airbnb_df$host_is_superhost == 1])

qqnorm(airbnb_df$review_scores_rating)
qqline(airbnb_df$review_scores_rating)


# Anderson-Darling normality test
install.packages("nortest")
library(nortest)
ad.test(airbnb_df$review_scores_rating)
#not normally distributed


model_1 <- lm (superhost_true ~ review_scores_rating ,data = airbnb_df)

coeftest(model_1, vcov = vcovHC(model_1, type="HC1"))

plot(review_scores_rating~superhost_true, data = airbnb_df)
